...
class Agendamento {
    constructor(id_agendamento, data, id_cliente, id_veiculo, id_servico, status) {
    ...
    static async salvar(agendamento) {
        const query = `INSERT INTO agendamento (data, id_cliente, id_veiculo, id_servico, status) 
                       VALUES (?, ?, ?, ?, ?)`;
        const values = [
            agendamento.data,
    ...
    static async atualizar(agendamento) {
        const query = `UPDATE agendamento SET data = ?, id_cliente = ?, id_veiculo = ?, id_servico = ?, status = ? 
